...
public class Chunk<T> : Entity<Guid>
{
    ...
    private Chunk(Guid id, int index, T artifact, ChunkSource source) : base(id)
    {
        Index = index;
        Artifact = artifact;
        Source = source;
    ...
    public static Chunk<T> Create(int index, T artifact, ChunkSource source = ChunkSource.Structured) =>
        new(Guid.NewGuid(), index, artifact, source);
